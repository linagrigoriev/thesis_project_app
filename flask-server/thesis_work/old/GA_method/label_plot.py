    ...
    if (x_choice == 1):
        ...
        for course_seminar_id, slot_id in solution_items:
            if course_seminar_id[:3] == choice_id:
                professor_id = lectures_seminars_data[course_seminar_id][1]
                if course_seminar_id[-2] == "G":
                    study_program_id_data.append([next((item[1] for item in professors_data if item[0] == professor_id), None), slot_id[:-4], slot_id[-3], slot_id[-1], 1, course_seminar_id[-1]])
                else:
                    study_program_id_data.append([next((item[1] for item in professors_data if item[0] == professor_id), None), slot_id[:-4], slot_id[-3], slot_id[-1], 0, 0])
        ...
        if study_program_id_data != []:
            ...
            for element in study_program_id_data:
                professor_name, room_id, timeslot_id, day_id, course_seminar, group_id_number = element
                if course_seminar:
                    label = f"Seminar cu {professor_name}, grupa {group_id_number}, sala {room_id[-1]}"
                else:
                    label = f"Curs cu {professor_name}, sala {room_id[-1]}"
                start_time = next((item[1] for item in timeslots_data if item[0] == int(timeslot_id)), None)
                end_time = next((item[2] for item in timeslots_data if item[0] == int(timeslot_id)), None)
                day = next((item[1] for item in days_data if item[0] == int(day_id)), None)
        ...
        for course_seminar_id, slot_id in solution_items:
            if slot_id[:-4] == choice_id:
                professor_id = lectures_seminars_data[course_seminar_id][1]
                if course_seminar_id[-2] == "G":
                    room_id_data.append([next((item[1] for item in professors_data if item[0] == professor_id), None), course_seminar_id, slot_id[-3], slot_id[-1], 1, course_seminar_id[-1]])
                else:
                    room_id_data.append([next((item[1] for item in professors_data if item[0] == professor_id), None), course_seminar_id, slot_id[-3], slot_id[-1], 0, 0])
        ...
        if room_id_data != []:
            ...
            for element in room_id_data:
                professor_name, course_seminar_id, timeslot_id, day_id, course_seminar, group_id_number = element
                if course_seminar:
                    label = f"Seminar cu {professor_name}, specializarea{course_seminar_id[:2]}, anul {course_seminar_id[2]}, grupa {course_seminar_id[-1]}"
                else:
                    label = f"Curs cu {professor_name}, specializarea{course_seminar_id[:2]}, anul {course_seminar_id[2]}"
                start_time = next((item[1] for item in timeslots_data if item[0] == int(timeslot_id)), None)
                end_time = next((item[2] for item in timeslots_data if item[0] == int(timeslot_id)), None)
                day = next((item[1] for item in days_data if item[0] == int(day_id)), None)
...
def pass_data_to_generate_plot(solution_items, lectures_seminars_data):
    ...
    if answer == 1:
        lecturers_list = [i[0]+"  "+i[1] for i in professors_data]
